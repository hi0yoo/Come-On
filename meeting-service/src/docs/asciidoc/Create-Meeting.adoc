[[Create-Meeting]]
== 1. 모임 생성 - **/meetings**

[[Create-Normal]]
=== 1) 정상 흐름
==== 요청 예시
include::{snippets}/meeting-create-normal/http-request.adoc[]

==== 요청 파라미터
include::{snippets}/meeting-create-normal/request-parameters.adoc[]
include::{snippets}/meeting-create-normal/request-parts.adoc[]
필수값이 true인 파라미터는 무조건 multipart/form-data에 포함되어 있어야 합니다.

==== 응답 예시
include::{snippets}/meeting-create-normal/http-response.adoc[]
생성에 성공한 모임의 식별자 값을 응답합니다.

[[Create-Error]]
=== 2) 예외 - 필수값이 없는 경우
==== 요청 예시
include::{snippets}/meeting-create-badrequest/http-request.adoc[]
위와 같이 title 값만 요청 데이터로 보낼 경우에는, 필수 값이 true인 파라미터들이 없기 때문에 예외를 발생시킵니다.

==== 응답 예시
include::{snippets}/meeting-create-badrequest/http-response.adoc[]

==== 응답 필드
include::{snippets}/meeting-create-badrequest/response-fields-data.adoc[]