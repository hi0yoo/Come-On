[[user-modify]]
== 2. 유저 수정 - **/meetings/{meetingId}/users/{codeId}**
...
변경하려는 회원이 해당 모임에 가입되어있지 않기 때문에 NOT_FOUND를 응답합니다.
...
[[user-modify-error-not-host]]
=== 7) 예외 - 회원의 역할이 HOST가 아닌 경우
==== 요청 예시
include::{snippets}/user-modify-error-not-host/http-request.adoc[]
위의 예시는 모임에 가입되어 있지만 HOST 역할이 아닌 회원의 토큰으로 요청을 보낸 경우입니다.

==== 응답 예시
include::{snippets}/user-modify-error-not-host/http-response.adoc[]

==== 응답 필드
include::{snippets}/user-modify-error-not-host/response-fields-data.adoc[]
