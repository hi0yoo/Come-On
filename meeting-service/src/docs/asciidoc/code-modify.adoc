...
위의 예시는 30번 식별자를 가진 모임 코드가 아직 만료되지 않았는데 갱신 요청을 보낸 경우입니다.
...
위의 예시는 5번 식별자를 가진 모임이 없는데 모임 코드 갱신 요청을 보낸 경우입니다.
...
[[code-modify-error-not-host]]
=== 6) 예외 - 회원의 역할이 HOST가 아닌 경우
==== 요청 예시
include::{snippets}/code-modify-error-not-host/http-request.adoc[]
위의 예시는 HOST 역할이 아닌 회원의 토큰으로 요청을 보낸 경우입니다.

==== 응답 예시
include::{snippets}/code-modify-error-not-host/http-response.adoc[]

==== 응답 필드
include::{snippets}/code-modify-error-not-host/response-fields-data.adoc[]
