...
include::{snippets}/user-save/success/request-fields.adoc[]
...
여러 유저 리스트를 조회하기 위해 다음 경로로 요청합니다.
...
요청을 성공적으로 처리하면 다음과 같은 응답을 반환합니다. +
탈퇴한 회원의 경우, ``nickname``, ``profileImgUrl`` 필드가 존재하지 않습니다.
...
include::{snippets}/user-list/fail/http-request.adoc[]

==== 응답 예시

include::{snippets}/user-list/fail/http-response.adoc[]

==== 응답 필드

include::{snippets}/user-list/fail/response-fields-data.adoc[]
...
단일 회원 정보를 조회하기 위해서는 다음과 같이 요청합니다.
...
include::{snippets}/user-modify/fail_1/http-response.adoc[]

==== 응답 필드

include::{snippets}/user-modify/fail_1/response-fields-data.adoc[]
